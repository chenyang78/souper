...
void CandidateReplacement::printLHS(llvm::raw_ostream &Out) const {
  PrintReplacementLHS(Out, BPCs, PCs, Mapping.LHS);
}

void CandidateReplacement::print(llvm::raw_ostream &Out) const {
  PrintReplacement(Out, BPCs, PCs, Mapping);
}
...
struct ExprBuilder {
  ...
  Inst *build(Value *V);
  InstMapping getOnePC(BasicBlock *BB, BasicBlock *Pred);
  void addPathConditions(std::vector<InstMapping> &PCs, BasicBlock *BB);
  void addBlockPathConditions(BlockPCs &BPCs,
                              std::vector<InstMapping> &CurrentPCs,
                              std::unordered_set<Block *> &AddedBlocks,
                              BasicBlock *BB);
  Inst *get(Value *V);
};
...
void emplace_back_dedup(std::vector<InstMapping> &PCs, InstMapping PC) {
  // Skip dummy PC generated by getOnePC.
  if (!PC.LHS)
    return;
  for (auto &I : PCs) {
    if (I.LHS == PC.LHS && I.RHS == PC.RHS)
      return;
  }
  PCs.emplace_back(PC.LHS, PC.RHS);
}

InstMapping ExprBuilder::getOnePC(BasicBlock *BB, BasicBlock *Pred) {
  if (auto Branch = dyn_cast<BranchInst>(Pred->getTerminator())) {
    if (Branch->isConditional()) {
      return InstMapping(get(Branch->getCondition()),
    ...
    if (Case) {
      return InstMapping(Cond, get(Case));
    } else {
      // default
      ...
      return InstMapping(IC.getInst(Inst::And, 1, Cases),
                         IC.getConst(APInt(1, true)));
    }
  }
  // Return a dummy mapping.
  return InstMapping();
}
...
void ExprBuilder::addBlockPathConditions(
  ...
  if (auto Pred = BB->getSinglePredecessor()) {
    // Skip BBs with single predecessor. 
    auto CurrentPred = Pred;
    while (auto P = Pred->getSinglePredecessor()) 
      Pred = P;

    std::vector<InstMapping> NewPCs;
    addBlockPathConditions(BPCs, NewPCs, AddedBlocks, Pred);

    InstMapping PC = getOnePC(BB, CurrentPred);
    if (PC.LHS)
      CurrentPCs.emplace_back(PC);
  } else {
    ...
    //        \  /
    //         B4
    if (AddedBlocks.count(BI.B))
      return;

    std::vector<InstMapping> NewPCs;
    // We don't know which predecessor would be chosen, so we can
    // add path condition to BlockPCs from now on.
    // BlockPCs is added only if we succeeded on all predecessors.
    for (auto Pred : BI.Preds) {
      addBlockPathConditions(BPCs, NewPCs, AddedBlocks, Pred);
    }
    // If blockpc(s) do not match incoming branches, just return.
    // We can't always get PC along a predecessor. For example, 
    // We may hit a loop entry point, or we may have two consecutive
    // nodes with multiple incoming branches such as:
    //      \  /
    //       B1
    //      /  \  /
    //     B2   B3 
    //         /  \  /
    //             B4
    if (NewPCs.size() != BI.Preds.size())
      return;
    
    BPCs.emplace_back(BI.B, NewPCs);
    AddedBlocks.insert(BI.B);
  }
}
...
void ExtractExprCandidates(Function &F, const LoopInfo *LI,
  ...
  for (auto &BB : F) {
    ...
    if (!BCS->Replacements.empty()) {
      ...
      std::vector<InstClasses> BlockVars;
      std::vector<std::vector<Inst *> > BlockPCSets;
      // Only need to start from a basic block with multiple predecessors.
      if (!BB.getSinglePredecessor()) {
        std::vector<InstMapping> CurrentPCs;
        std::unordered_set<Block *> AddedBlocks;
        EB.addBlockPathConditions(BCS->BPCs, CurrentPCs, AddedBlocks, &BB);
        for (auto &BPC : BCS->BPCs) {
          InstClasses BVs;
          BlockPCSets.emplace_back(AddPCSets(BPC.PCs, BVs));
          BlockVars.emplace_back(BVs);
        }
      }

      for (auto &R : BCS->Replacements) {
        R.PCs = GetRelevantPCs(BCS->PCs, PCSets, Vars, R.Mapping);
        for (unsigned I = 0; I < BCS->BPCs.size(); ++I) {
          auto BPC = BCS->BPCs[I];
          std::vector<InstMapping> RelBPC = 
              GetRelevantPCs(BPC.PCs, BlockPCSets[I], BlockVars[I], R.Mapping);
          assert(BPC.B && "NULL Block for BlockPC?");
          // If any blockpc is irrelevant to R.Mapping.LHS, 
          // skip the entire set of blockpc(s). Thus, blockpc(s) have the same
          // semantics as pc. 
          if (RelBPC.size() == BPC.PCs.size())
            R.BPCs.emplace_back(BPC.B, RelBPC);
        }
      }
